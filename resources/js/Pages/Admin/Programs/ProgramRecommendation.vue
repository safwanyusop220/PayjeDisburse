...
const approveProgram = (id) => {
    try{
        form.put(route('programs.approveRecommendation', props.program?.id), {
            onSuccess: (page) => {
                Swal.fire(
                    'Success',
                    'Program Has Successfully Recommended!',
                    'success'
                )
            showConfirmApproveModal.value = false;
...
const { value } = await Swal.fire({
  ...
  try {
    ...
    await form.post(route('programs.rejectRecommendation', props.program?.id), {
    value, 
    onSuccess: (page) => {
        Toast.fire({
        icon: 'success',
...
<template>
    <Head title="Program Recommendation" />
                ...
                <div class="flex justify-between pt-4">
                    <nav class="text-sm font-semibold" aria-label="Breadcrumb">
                        <ol class="list-none p-0 inline-flex">
                            ...
                            <li class="flex items-center">
                                <BreadcrumbActive :href="route('programs.waitingRecommendation')">Waiting Recommendation</BreadcrumbActive>
                            </li>
                            <li class="flex items-center">
                    ...
                    <form @submit.prevent="submit">
                        ...
                        <div class="flex justify-end space-x-2 mb-2">
                            <button v-if="disableButtonReject"
                                    ...
                                    class="flex items-center py-2 px-4 text-xs text-white font-mono bg-danger-button hover:bg-danger-button-hover rounded-md"
                                    @click="reject"
                                    >
                                ...
                                <a v-if="!disableButtonReject"
                                    class="flex items-center py-2 px-4 text-xs text-white font-mono bg-danger-button-hover rounded-md"
                                    >
                                    <IcCancel class="text-lg mr-0.5" />
                                    Reject
                                </a>
                        ...
                        <Modal :show="showConfirmApproveModal" @close="closeModalApprove">
                            <div class="p-6">
                                ...
                                <div class="flex justify-center">
                                    ...
                                    Are you sure you want to Recommend this Program?
                                    </h2>
                                </div>
