...
const approveProgram = (selectedItems) => {
    try {
        selectedItems.forEach((id) => {
        form.put(route("programs.approveRecommendation", id), {
            onSuccess: (page) => {
            Swal.fire({
                width: 400,
                height: 100,
...
const rejectAllocation = async (selectedItems) => {
    ...
    try {
        selectedItems.forEach((id) => {
            form.post(route("programs.rejectRecommendation", id), {
            onSuccess: (page) => {
                Toast.fire({
                icon: "success",
...
watch(search, (value) =>{
    router.get("/programs/waiting-recommendation", {search: value, perPage: perPage.value},
    {
        preserveState: true,
        replace: true
    });
...
function getPrograms() {
    router.get(
        "/programs/waiting-recommendation",
                ...
                <Content>
                    <form @submit.prevent="submit">
                        <div class="flex justify-between">
                            <span class="font-medium text-primary-text text-lg mt-2">Program Recommendation</span>
                            ...
                            <div class="flex justify-end space-x-2 -mt-2">
                                ...
                                <button v-if="totalSelected == 0"
                                    disabled
                                    class="w-[100px] justify-center my-3 flex items-center py-1.5 px-4 text-xxs text-white font-mono bg-danger-button-hover rounded-md"
                                    @click="reject">
                                    <IcCancel class="text-lg mr-0.5" />
                                    Reject 
                                        <template v-if="totalSelected != 0">
                                ...
                                <button v-if="totalSelected != 0"
                                    :disabled="form.processing"
                                    class="w-[100px] justify-center my-3 flex items-center px-2 text-xxs text-white font-mono bg-success-button hover:bg-success-button-hover rounded-md"
                                    @click="approve">
                                ...
                                <button v-if="totalSelected == 0"
                                    disabled
                                    class="w-[100px] justify-center my-3 flex items-center px-2 text-xxs text-white font-mono bg-success-button-hover rounded-md"
                                   >
                                    <BxCheckDouble class="text-lg mr-0.5" />
                                    Recommend 
                                </button>
                                            ...
                                            <TableDataCellView class="">
                                                <div class="row">
                                                    <div class="col grid-cols-1 justify-items-center flex justify-center">
                                                        <Link :href="route('programs.programRecommendation', program.id)" class="flex items-center text-xxs-custom space-x-2 px-2 py-1 bg-blue-500 text-white hover:bg-blue-600 rounded-md">
