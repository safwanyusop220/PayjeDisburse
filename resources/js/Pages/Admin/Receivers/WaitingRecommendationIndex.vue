...
const approveReceiver = (selectedItems) => {
    try {
        ...
        form.put(route("receivers.approveRecommendation", id), {
            onSuccess: (page) => {
            Swal.fire({
                width: 400,
                height: 100,
...
const rejectReceiver = async (selectedItems) => {
    ...
    try {
        selectedItems.forEach((id) => {
            form.post(route("receivers.rejectRecommendation", id), {
            onSuccess: (page) => {
                Toast.fire({
                icon: "success",
...
watch(search, (value) =>{
    router.get("/waiting-recommendation", {search: value, perPage: perPage.value},
    {
        preserveState: true,
        replace: true
    });
...
<template>
    <Head title="Waiting Recommendation" />
                ...
                <Content>
                    <form @submit.prevent="submit">
                        <div class="flex justify-between">
                            <span class="font-medium text-primary-text text-lg mt-2">Receiver Waiting Recommendation</span>
                                        ...
                                        <TableDataCellView class="w-1/12">
                                            <div class="row">
                                                <div class="grid grid-cols-2 justify-items-center">
                                                    ...
                                                    <Link :href="route('receivers.receiverRecommendation', receiver.id)" class="flex items-center text-xxs-custom space-x-2 px-2 py-1 bg-blue-500 text-white hover:bg-blue-600 rounded-md">
